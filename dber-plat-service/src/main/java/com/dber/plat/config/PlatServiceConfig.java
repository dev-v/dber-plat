...
public class PlatServiceConfig {
	@Bean
	@ConfigurationProperties(prefix = "dber.plat.service.mysql")
	public JdbcPoolConfig platJdbcPoolConfig() {
		return new JdbcPoolConfig();
	}

	@Bean
	public DataSource platDataSource() {
		DataSource platDataSource = DBUtil.dataSource(platJdbcPoolConfig());
